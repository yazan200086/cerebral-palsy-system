    ...
    def post(self, request, *args, **kwargs):
        ...
        if test == 'info_test':
            try:
                ...
                kid=models.WechslerChild.objects.get(id=id)
                kid.vrebal_scal=randrange(30)+kid.vrebal_scal
                kid.save()
        ...
        if test == 'similarity_test':
            ...
            try:
                ...
                kid=models.WechslerChild.objects.get(id=id)
                kid.practical_scale=randrange(30)+kid.practical_scale
                kid.save()
                return JsonResponse ({'status':'done with randome values'})
...
class WechslerDiagnose(APIView):
        ...
        def get(self, request, *args, **kwargs):
            ...
            if kid.IQ in range(20,70):
                kid.diagnose='تاخر عقلي شديد'
            elif kid.IQ in range(70,84):
                kid.diagnose='بطء تعلم'
            elif kid.IQ in range(85,1000):
                kid.diagnose='طبيعي'
        ...
        def post(self, request, *args, **kwargs):
            id = self.request.headers.get('id')
            kid=models.WechslerChild.objects.filter(child__id=id).last()
...
class WechslerChildGet(viewsets.ReadOnlyModelViewSet):   
    ...
    def get_queryset(self):
        ...
        kid=models.WechslerChild.objects.filter(child__id=child_id)    
        return kid
...
def CalculateLddrs(request):
    ...
    # if test_name=='صعوبات الكتابة':
    #     child.Lddrs_diagnose_writing_difficulties=diagnose
    #     child.save()
    # elif test_name=='صعوبات الاستماع':
    #     child.Lddrs_diagnose_listening_difficulties=diagnose
    #     child.save()
    return Response({'diagnose':diagnose})
